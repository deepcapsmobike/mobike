...
class Conv3DCaps(layers.Layer):
    ...
    def call(self, inputs):
        if self.r_num == 1:
            ...
            outputs = K.conv3d(K.reshape(inputs, (-1, self.h_i, self.w_i,
                                                  self.ch_i * self.n_i)),
...
class Conv2DCaps(layers.Layer):
    ...
    def call(self, inputs):
        ...
        if self.r_num == 1:
            ...
            # print(inputs.shape)
            # print(self.w.shape)
            # print(K.reshape(self.w, self.kernel_size +
            #                              (self.ch_i * self.n_i, self.ch_j * self.n_j)))
            # print(self.ch_i)
            # print(self.n_i)
            outputs = K.conv2d(K.reshape(inputs, (-1, self.h_i, self.w_i,
                               ...
                               padding=self.padding,
                               dilation_rate=self.dilation_rate)
            # print(outputs.shape) # (256, 126, 3, 128) 
            outputs = squeeze(K.reshape(outputs, ((-1, self.h_j, self.w_j,
                                                   self.ch_j, self.n_j))))
            # print(outputs.shape) # (256, 126, 3, 32, 4)
        # print("!!!!!!!!!!!!!!!")
        return outputs
